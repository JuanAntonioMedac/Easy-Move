...
@section('title', 'Nuevo servicio · Admin EasyMove')
...
<div class="mb-8">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Nuevo servicio</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Añade un servicio al catálogo</p>
        </div>
    </div>
</div>
    ...
    <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-5">
        ...
        <div>
            ...
            <select id="id_proveedor" name="id_proveedor" required
                ...
                @foreach ($proveedores as $proveedor)
                    <option value="{{ $proveedor->id_proveedor }}" @selected(old('id_proveedor') == $proveedor->id_proveedor)>{{ $proveedor->nombre }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="id_tipo_servicio" name="id_tipo_servicio" required
                ...
                @foreach ($tiposServicios as $tipo)
                    <option value="{{ $tipo->id_tipo_servicio }}" @selected(old('id_tipo_servicio') == $tipo->id_tipo_servicio)>{{ $tipo->nombre }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion" rows="4"
                      class="w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-950/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all @error('descripcion') border-rose-500 @else border-slate-200 dark:border-slate-700 @enderror">{{ old('descripcion') }}</textarea>
