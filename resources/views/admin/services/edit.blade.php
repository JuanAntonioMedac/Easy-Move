...
<div class="mb-8">
    ...
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Servicio</h1>
</div>

@include('shared.alerts')

<div class="bg-white dark:bg-slate-900 rounded-lg shadow-md p-8 border border-gray-200 dark:border-slate-800 max-w-2xl">
    <form method="POST" action="{{ route('admin.services.update', $servicio) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select id="id_proveedor" name="id_proveedor" required
                ...
                @foreach ($proveedores as $proveedor)
                    <option value="{{ $proveedor->id_proveedor }}" @selected(old('id_proveedor', $servicio->id_proveedor) == $proveedor->id_proveedor)>
                        {{ $proveedor->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="id_tipo_servicio" name="id_tipo_servicio" required
                ...
                @foreach ($tiposServicios as $tipo)
                    <option value="{{ $tipo->id_tipo_servicio }}" @selected(old('id_tipo_servicio', $servicio->id_tipo_servicio) == $tipo->id_tipo_servicio)>
                        {{ $tipo->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion" rows="4"
                      class="w-full px-4 py-2 border border-gray-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-600 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $servicio->descripcion) }}</textarea>
