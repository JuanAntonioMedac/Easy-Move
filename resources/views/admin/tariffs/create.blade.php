...
@section('title', 'Nueva tarifa · Admin EasyMove')
...
<div class="mb-8">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Nueva tarifa</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Define una nueva tarifa con precio y disponibilidad</p>
        </div>
    </div>
</div>
...
<div class="max-w-3xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8">
    <form method="POST" action="{{ route('admin.tariffs.store') }}" class="space-y-5">
        ...
        <div>
            ...
            <select id="id_servicio" name="id_servicio" required
                ...
                @foreach ($servicios as $servicio)
                    <option value="{{ $servicio->id_servicio }}" @selected(old('id_servicio') == $servicio->id_servicio)>{{ $servicio->nombre_servicio }} ({{ $servicio->proveedor->nombre }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" id="nombre_tarifa" name="nombre_tarifa" value="{{ old('nombre_tarifa') }}" required placeholder="Ej: Plan Básico, Premium…"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="precio" name="precio" value="{{ old('precio') }}" step="0.01" min="0.01" required placeholder="29.99"
            ...
            <div>
                ...
                <input type="text" id="unidad_precio" name="unidad_precio" value="{{ old('unidad_precio') }}" required placeholder="mes, año, GB, Mbps…"
        ...
        <div>
            ...
            <select id="permanencia" name="permanencia"
                ...
                <option value="1mes" @selected(old('permanencia') == '1mes')>1 mes</option>
                <option value="3meses" @selected(old('permanencia') == '3meses')>3 meses</option>
                <option value="6meses" @selected(old('permanencia') == '6meses')>6 meses</option>
                <option value="12meses" @selected(old('permanencia') == '12meses')>12 meses</option>
                <option value="sin_permanencia" @selected(old('permanencia') == 'sin_permanencia')>Sin permanencia</option>
            </select>
        </div>
        ...
        <div>
            ...
            <textarea id="condiciones" name="condiciones" rows="4" placeholder="Detalles, condiciones especiales, energía verde…"
                      class="w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-950/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all @error('condiciones') border-rose-500 @else border-slate-200 dark:border-slate-700 @enderror">{{ old('condiciones') }}</textarea>
        ...
        <div>
            ...
            <input type="url" id="url_oferta_externa" name="url_oferta_externa" value="{{ old('url_oferta_externa') }}" placeholder="https://ejemplo.com/oferta"
                ...
                @forelse ($ubicaciones as $ubicacion)
                    <label class="cursor-pointer">
                        <input type="checkbox" name="ubicaciones[]" value="{{ $ubicacion->id_ubicacion }}"
                               @checked(in_array($ubicacion->id_ubicacion, old('ubicaciones', [])))
                               class="peer hidden">
        ...
        <div class="flex gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
            <button type="submit" class="btn-brand ring-brand flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-white font-bold">
                <i class="bi bi-check-lg"></i>Crear tarifa
            </button>
