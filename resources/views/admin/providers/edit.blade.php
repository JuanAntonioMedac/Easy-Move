...
<div class="mb-8">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Editar proveedor</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Modificando <span class="font-bold text-slate-700 dark:text-slate-200">{{ $proveedor->nombre }}</span></p>
        </div>
    </div>
</div>
...
<div class="max-w-2xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-8">
    <form method="POST" action="{{ route('admin.providers.update', $proveedor) }}" enctype="multipart/form-data" class="space-y-5">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <input type="text" id="tipo_proveedor" name="tipo_proveedor" value="{{ old('tipo_proveedor', $proveedor->tipo_proveedor) }}" required placeholder="Ej: Telecomunicaciones, Energía…"
        ...
        <div>
            ...
            <input type="url" id="web" name="web" value="{{ old('web', $proveedor->web) }}" placeholder="https://ejemplo.com"
        ...
        <div>
            ...
            @if ($proveedor->logo)
                <div class="mb-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 flex items-center gap-4">
                    <img src="{{ $proveedor->logo_url }}" alt="{{ $proveedor->nombre }}" class="w-16 h-16 rounded-xl object-contain bg-white dark:bg-slate-900 p-1">
                    <div>
                        <p class="text-sm font-bold text-slate-700 dark:text-slate-200">Logo actual</p>
                        @if ($proveedor->isExternalLogoUrl())
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-100 dark:bg-sky-950/50 text-sky-700 dark:text-sky-300 mt-1"><i class="bi bi-link-45deg"></i>URL externa</span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 mt-1"><i class="bi bi-folder"></i>Local</span>
                        @endif
                    </div>
                </div>
            @endif
            ...
            <div id="fileInput">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1.5">Máximo 5MB · Dejar vacío para mantener el actual</p>
            </div>
        ...
        <div class="flex items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
            <input type="checkbox" id="api_disponible" name="api_disponible" @checked(old('api_disponible', $proveedor->api_disponible))
            ...
            <label for="api_disponible" class="flex-1 text-sm font-semibold text-slate-700 dark:text-slate-300">
                ...
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-normal mt-0.5">El proveedor expone una API para tarifas en tiempo real</span>
            </label>
        </div>
